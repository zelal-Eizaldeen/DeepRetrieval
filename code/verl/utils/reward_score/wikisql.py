...
from src.sql.wikisql import WikiSQLDatabaseSearcher
...
def get_searcher():
    global _searcher
    _searcher = WikiSQLDatabaseSearcher()
    return _searcher
...
def execute_sql(sql_query, db_path, db_id):
    """Retrieve items from the search system."""
    searcher = get_searcher()
    results = searcher.search(sql_query, db_path, db_id)
    return results
    
def calculate_answer_score(pred_sql, gold_sql, db_path, db_id, do_print=False):
    """Calculate answer score based on final_prediction idx."""
    try:
        pred_results = execute_sql(pred_sql, db_path, db_id)
        gold_results = execute_sql(gold_sql, db_path, db_id)
...
def compute_score(solution_str, ground_truth, data_source, db_path, db_id, format_reward=0.1, answer_reward=1.):
    ...
    if format_correct and answer_text:
        ...
        answer_score = calculate_answer_score(pred_sql, gold_sql, db_path, db_id, do_print)
